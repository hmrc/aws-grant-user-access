locals {
  default_policy_arns = concat([aws_iam_policy.build.arn, aws_iam_policy.build_core.arn], var.project_iam_policy_arns)
...
data "aws_iam_policy_document" "build_core" {
  ...
  statement {
    ...
    resources = [
      module.builder_bucket.kms_key_arn
    ]
  }
  ...
  statement {
    ...
    resources = [
      "${module.builder_bucket.arn}/*/source_out/*",
      "${module.builder_bucket.arn}/*/build_outp/*",
    ]
  }
